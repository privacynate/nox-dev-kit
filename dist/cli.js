...
async function main() {
    ...
    fs.writeFileSync(path.join(commandsDir, "nox-scaffold.md"), `Generate a new Nox confidential smart contract based on my description. Use the patterns and rules from CLAUDE.md in this project. Always include proper Nox.allowThis() calls, ACL management, and follow the anti-patterns list. Use pragma solidity ^0.8.28 and the correct import paths.`);
    fs.writeFileSync(path.join(commandsDir, "nox-lint.md"), `Review the current Nox smart contract for common mistakes. Check for:
...
Report each issue with line number and fix.`);
    fs.writeFileSync(path.join(commandsDir, "nox-deploy.md"), `Compile and deploy the current Nox contract to Arbitrum Sepolia. Steps:
...
Use the PRIVATE_KEY and ARBITRUM_SEPOLIA_RPC from .env file.`);
    fs.writeFileSync(path.join(commandsDir, "nox-audit.md"), `Run a security audit of the current Nox contract. Check for:
